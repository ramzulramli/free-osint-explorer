...
export async function enrichWithImages(result,env={},provider=null){const images=[],seen=new Set();
  try{const searched=await imageSearch(result?.subject||result?.query,env,IMAGE_LIMITS.maxSearchImages);for(const item of searched.results||[]){if(images.length>=IMAGE_LIMITS.maxImages)break;const url=cleanImageUrl(item.url,item.sourceUrl||result?.subject);if(!url)continue;addImage(images,seen,{url,thumbnail:item.thumbnail?cleanImageUrl(item.thumbnail,item.sourceUrl||url):null,sourceUrl:item.sourceUrl||url,sourceTitle:item.title||"Image search result",caption:item.title||"Related image",provider:searched.provider});}}
  catch{}
