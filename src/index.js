...
function extractReadableText(html){
  return stripHtml(String(html||"").replace(/<script[\s\S]*?<\/script>/gi," ").replace(/<style[\s\S]*?<\/style>/gi," ").replace(/<noscript[\s\S]*?<\/noscript>/gi," ").replace(/<svg[\s\S]*?<\/svg>/gi," ").replace(/<template[\s\S]*?<\/template>/gi," ").replace(/<nav[\s\S]*?<\/nav>/gi," ").replace(/<footer[\s\S]*?<\/footer>/gi," ").replace(/<!--[\s\S]*?-->/g," "));
}
...
function isLikelyPersonName(value){
  const words=value.trim().split(/\s+/);if(words.length<2||words.length>5||value.length<5||value.length>80)return false;
  const normalized=normalizeEntity(value);if(COMMON_NON_PERSON_PHRASES.has(normalized)||MALAYSIAN_LOCATIONS.some(x=>normalizeEntity(x)===normalized))return false;
  const lowerWords=words.map(w=>w.replace(/[^a-zA-ZÀ-ÿ'-]/g,"").toLowerCase());if(lowerWords.some(w=>COMMON_NON_PERSON_WORDS.has(w)||COMMON_ORGANISATION_WORDS.has(w)))return false;
  return words.filter(w=>/^[A-ZÀ-Ý][a-zà-ÿ'-]*$/.test(w)).length>=2;
}
function extractPersonCandidates(text){
  const out=[],seen=new Set(),regex=/\b[A-ZÀ-Ý][a-zà-ÿ'-]+(?:\s+(?:bin|binti|[A-ZÀ-Ý][a-zà-ÿ'-]+)){1,4}\b/g;let match;
...
function extractOrganisationCandidates(text){
  const out=[],seen=new Set(),regex=/\b[A-ZÀ-Ý][A-Za-zÀ-ÿ&.'-]*(?:\s+[A-ZÀ-Ý][A-Za-zÀ-ÿ&.'-]*){0,2}\s+(?:Berhad|Bhd|Sdn Bhd|Foundation|Association|University|Corporation|Company|Limited|Ltd|FA|F\.A\.|FC|F\.C\.)\b/g;let match;
...
function extractKeywords(text){
  const stopWords=new Set(["the","and","for","with","from","this","that","have","has","are","was","were","you","your","our","their","about","into","more","www","http","https"]),counts=new Map();
  for(const word of text.toLowerCase().match(/\b[a-z][a-z0-9_-]{3,}\b/g)||[])if(!stopWords.has(word))counts.set(word,(counts.get(word)||0)+1);
  return [...counts.entries()].sort((a,b)=>b[1]-a[1]).slice(0,20).map(([value,count])=>({type:"keyword",value,normalized:value,confidence:Math.min(.95,.45+count*.05),evidence:"Repeated content keyword"}));
}
...
export { extractEntityCandidates, extractTitle, extractReadableText, extractLinks };

export default { async fetch(request,env){
  const url=new URL(request.url);
  if(url.pathname==="/")return Response.json({name:"Free OSINT Explorer",status:"online",version:"0.4.0"});
