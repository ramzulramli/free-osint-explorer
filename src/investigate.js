...
const LIMITS = {
  maxSearchResults: 5, maxPages: 5, maxEntitiesPerSource: 50,
  maxRankedEntities: 50, maxDiscoveries: 25, maxDepth: 2,
  maxQueueItems: 5, maxSearchRequests: 5, maxVisitedQueries: 10
};

const PROFILE_HOSTS = new Map([
  ["linkedin.com", "LinkedIn"], ["my.linkedin.com", "LinkedIn"],
  ["shutterstock.com", "Shutterstock"], ["facebook.com", "Facebook"],
  ["instagram.com", "Instagram"], ["x.com", "X"], ["twitter.com", "X"],
  ["tiktok.com", "TikTok"], ["youtube.com", "YouTube"]
]);

const PERSON_NOISE = new Set([
  "certification", "certified", "tester", "number", "portfolio", "stock", "video",
  "collection", "collections", "technology", "services", "sdn", "bhd", "linkedin",
...
function cleanUrl(url) {
  try {
    const u = new URL(url);
    for (const key of ["msockid", "utm_source", "utm_medium", "utm_campaign", "utm_term", "utm_content"]) u.searchParams.delete(key);
    return u.toString();
...
async function fetchPage(url) {
  try {
    ...
    const response = await fetch(target.toString(), { headers: { "User-Agent": "Mozilla/5.0 (compatible; FreeOSINTExplorer/0.9)" } });
...
function buildPivotQueries(subject, accounts, organisations) {
  ...
  const push = q => { const n = normalize(q); if (!n || seen.has(n)) return; seen.add(n); queries.push(q); };
...
async function investigate(request, env) {
  ...
  const provider = url.searchParams.get("provider")?.trim().toLowerCase() || null, startedAt = Date.now();
  const state = { visited: new Set(), searchRequests: 0, pagesRead: 0, subject: query, skippedSearches: [] }, investigations = [];
  ...
  if (depth > 0 && root && state.searchRequests < LIMITS.maxSearchRequests) {
    ...
    const person = root.entities.find(e => e.type === "person_candidate" && isPerson(e, query)) || root.entities.find(e => e.type === "person_candidate" && normalize(e.value) === normalize(query));
    if (person) state.subject = person.value;
    for (const pivot of buildPivotQueries(state.subject, accountEntities, organisationEntities)) {
      if (state.searchRequests >= LIMITS.maxSearchRequests) break;
      const inv = await investigateQuery(pivot, state, env, provider, 1); if (inv) investigations.push(inv);
    }
  }
...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === "/") return Response.json({ name: "Free OSINT Explorer", status: "online", version: "0.9.0", endpoints: ["/search", "/investigate"] });
